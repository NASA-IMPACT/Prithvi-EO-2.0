...
Prithvi 600M Huge to regress GPP flux at eddy covariance observation sites.
Author: Srija Chakraborty, Besart Mujeci, MERRA variables selection, flux inputs from Yanghui Kang
Usage:
    python main_flux_finetune_baselines_H.py
"""
import torch
...
from flux_regress_H import RegressionModel_flux
from flux_regress_H import prithvi
#import wandb
...
def main():
    with open('fluxconfig_H.yaml', 'r') as file:
    ...
    wt_file='/vol/cephfs/impact/srija/Prithvi-Global-downstream_v0/new_flood/checkpoint_H.pt'#checkpoint for prithvi-H
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_loss_train_running < best_track_loss:
            ...
            }, f'{checkpoint_dir}model_MSELoss_ep{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.pt')#drp{drp_rate}_v2
    ...
    np.savetxt(f"{metrics_dir}train_log_epoch_MSELoss_{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.csv",train_log_stack,fmt='%10.6f', delimiter=',', newline='\n', header='train_loss, mae_per_epoch, mse_per_epoch, r2')

    #save train loss convergence, add val if available
    plt.plot(loss_tot_running)
    plt.ylabel('mse loss (norm)')
    plt.xlabel('epoch')
    plt.savefig(f'{plots_dir}train_loss_MSELoss_{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.png', dpi=180)
    plt.close()
    ...
    np.savetxt(f"{metrics_dir}test_eval_MSELoss_ep{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.csv",test_stack,fmt='%10.6f', delimiter=',', newline='\n', header='pred_n, tar_n, pred, tar, mse,mae, rel_err')
    ...
    plt.savefig(f'{plots_dir}R2_test_MSELoss_{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.png', dpi=180)
    plt.close()
    ...
    np.savetxt(f"{metrics_dir}train_eval_MSELoss_ep{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.csv",test_stack,fmt='%10.6f', delimiter=',', newline='\n', header='pred_n, tar_n, pred, tar, mse,mae, rel_err')
    ...
    plt.savefig(f'{plots_dir}R2_train_MSELoss_{num_epochs}_lr{learning_rate}_{optim_name}_sc{sch}_yr{year_to_test}_H.png', dpi=180)
    plt.close()
