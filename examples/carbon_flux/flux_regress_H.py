...
class SimpleDecoder_comb_v2(nn.Module):
    def __init__(self, input_dim=1280, hidden_dim=256, output_dim=64):
        super(SimpleDecoder_comb_v2, self).__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim)# 1280 to 256; shape 10x1280 to 10x256
    ...
    def forward(self, x):
        x = self.relu(self.fc1(x))#shape 10x1280 to 10x256 ORG
...
class RegressionModel_flux(nn.Module):
    def __init__(self, prithvi_model):
        ...
        self.decoder = SimpleDecoder_comb_v2(input_dim=1280, hidden_dim=256, output_dim=64)
    ...
    def forward(self, im2d, pt1d):
        ...
        pri_enc = self.prithvi_model(im2d, None, None, 0)#batch x 6x1x1x50; none, none for loc, temporal, 0--mask; output: batch x 10 x 1280
